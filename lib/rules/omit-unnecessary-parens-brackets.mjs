    ...
    function checkUnnecessaryBraces(blockStatement, controllingNode) {
      ...
      context.report({
        ...
        fix(fixer) {
          ...
          // Check for comments between braces (heuristic check)
          const textBetweenBraces = sourceCode.text.slice(firstToken.range[1], lastToken.range[0])
          let containsSignificantContent = false
          try {
            // Attempt to parse, if it finds non-comment tokens, might be risky
            const tokensBetween = espree.tokenize(textBetweenBraces.trim(), parserConfig)
            if (tokensBetween.length > 0 && !tokensBetween.every(t => t.type === 'BlockComment' || t.type === 'LineComment')) {
              containsSignificantContent = true
            }
          } catch (e) {
            // Parsing error might indicate comments or complex structure, be safe
            containsSignificantContent = textBetweenBraces.trim().length > 0 // If not just whitespace, be careful
          }

          // Also check if text content differs significantly (another heuristic for comments)
          if (containsSignificantContent || textBetweenBraces.trim() !== sourceCode.getText(singleStatement).trim()) {
            // Try a simpler check: if the text between isn't exactly the inner text (modulo whitespace)
            const trimmedBetween = textBetweenBraces.trim()
            const trimmedInner = sourceCode.getText(singleStatement).trim()
            if (trimmedBetween !== trimmedInner) {
              console.warn("Cannot fix braces: Comments might exist around the single statement inside the block")
              return null
            }
            // If they *are* the same, maybe it's safe (less strict check)
          }
    ...
    function checkArrowFunctionBraces(node) {
      ...
      context.report({
        ...
        fix(fixer) {
          ...
          // Add a space before the expression if not adding parens
          const replacementText = (needsParens || expressionText.startsWith('(') ? '' : ' ') + expressionText

          return fixer.replaceTextRange(blockBody.range, replacementText)
        }
      })
    }
